...
from friday_automations.methods.methods import get_model_class
from friday_views.cbv_methods import render_template

"""Objectives and key result section"""


class Period(FridayModel):
...
class KeyResult(FridayModel):
    ...
    history = FridayAuditLog(bases=[FridayAuditInfo])
...
class EmployeeObjective(FridayModel):
    ...
    history = FridayAuditLog(bases=[FridayAuditInfo], related_name="history_set")
    archive = models.BooleanField(default=False)
    objects = FridayCompanyManager("employee_id__employee_work_info__company_id")
...
class Comment(models.Model):
    ...
    history = FridayAuditLog(excluded_fields=["comment"], bases=[FridayAuditInfo])
    objects = FridayCompanyManager(
...
class QuestionTemplate(FridayModel):
...
class Question(FridayModel):
    ...
    objects = FridayCompanyManager("template_id__company_id")

    def __str__(self):
        return self.question


class QuestionOptions(FridayModel):
    ...
    objects = FridayCompanyManager("question_id__template_id__company_id")


class Feedback(FridayModel):
...
class EmployeeBonusPoint(FridayModel):
    employee_id = models.ForeignKey(
        Employee,
        on_delete=models.DO_NOTHING,
