...
class RecruitmentCreationForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.instance.pk:
            self.fields["recruitment_managers"] = FridayMultiSelectField(
    ...
    def clean(self):
        if isinstance(self.fields["recruitment_managers"], FridayMultiSelectField):
...
class StageCreationForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        reload_queryset(self.fields)
        if not self.instance.pk:
            self.fields["stage_managers"] = FridayMultiSelectField(
                queryset=Employee.objects.filter(is_active=True),
                widget=FridayMultiSelectWidget(
                    filter_route_name="employee-widget-filter",
                    filter_class=EmployeeFilter,
    ...
    def clean(self):
        if isinstance(self.fields["stage_managers"], FridayMultiSelectField):
